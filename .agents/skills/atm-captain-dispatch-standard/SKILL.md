...
2. **小型 read-only 查詢**：例如 route check、scope check、`git log` / `git show --stat`、`rg`、單卡 collision 查詢、單一 closure packet 快速定位，優先派 **1-3 個內建 `gpt-5.4-mini` 子代理**平行查；`2 個` 只是常見預設，不是固定變數。不要優先外派到 001-007。
   * **等待型驗證補充**：若 route check、scope 收斂或 closure packet spot-check 必須等前一張外包大工單回報後才能做，這種不能平行的收斂單不要預先外派；等前置結果回來後，由 Captain 內建 `gpt-5.4-mini` sidecar 或 Captain 本地即時執行。
3. **中大型 read-only 盤點**：跨多 repo、大量檔案、需要長時間 inventory，才考慮 `006` 或外部 read-only 工單。
4. **依賴型收斂單內包**：若只讀驗證必須等待前一張外包大工單回報後才能執行（例如等 `001` 修卡後再做 route check、等 `007` 實作後再做 closure packet spot-check），不要預先外派成 `002/003/005` 工單；等前置結果回來後，由 Captain 內建 `gpt-5.4-mini` sidecar 或 Captain 本地即時執行。
5. **複雜審核 / scope audit / 架構裁決**：交給 `004` / `005` 或外部 AI，因為這類任務需要獨立判斷與可追溯審核。
6. **實作 / claim / commit / close**：交給最適合的外部執行代理，例如 `007`，或依 roster 現況分配。
...
* **等待型驗證不預派**：若驗證單不能與前置工單平行執行，只是前置完成後的收斂確認，預設不輸出成外部派工單；等前置回報回來後，用 Captain 內建 sidecar 立即跑只讀 check。

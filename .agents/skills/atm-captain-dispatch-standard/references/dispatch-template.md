...
```markdown
回報第一行必須是：代號：[人類 roster 代號，如 006]；模型：[建議執行模型]
...
### 請回報
請以固定編號條列回報，至少包含：1. 結論 2. 變更/觀察 3. 驗證 4. 狀態（staged / commit / push）5. 風險或缺口 6. 下一步 7. 大白話補一句；不得改成散文或自由發揮。
...
```markdown
回報第一行必須是：代號：[人類 roster 代號，如 007]；模型：[建議執行模型，如 Gemini 3.5 Flash]
...
```markdown
回報第一行必須是：代號：[人類 roster 代號，如 005]；模型：[建議執行模型]
...
```markdown
回報第一行必須是：代號：[人類 roster 代號，如 001]；模型：[建議執行模型]

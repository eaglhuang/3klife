 ...
 *   UCUFLogger.info(LogCategory.DATA, "可以移除的積木：", unused.map(b => b.texPath));
 *
...
export function getBlocksForEffect(effectId: string): VfxBlockDef[] {
    ...
    if (!ids) {
        UCUFLogger.warn(LogCategory.DATA, `[VfxUsageTable] effectId 未登錄: "${effectId}"，請在 vfx-usage-table.ts 的 VFX_EFFECT_TABLE 中補充宣告`);
        return [];
    }
    ...
    return ids.reduce<VfxBlockDef[]>((acc, id) => {
        ...
        if (!block) {
            UCUFLogger.warn(LogCategory.DATA, `[VfxUsageTable] blockId "${id}" 在 VFX_BLOCK_REGISTRY 中找不到，請確認 ID 是否正確`);
        } else {
            acc.push(block);
        }
        return acc;
...
export function findUnusedBlocks(): VfxBlockDef[] {
    ...
    if (unused.length > 0) {
        ...
        UCUFLogger.info(LogCategory.DATA, '[VfxUsageTable] ✅ 所有積木都已被至少一個 effect 引用');
    }

...
/**
 * BattleHudOverlayCompositePanel — composite-panel family Panel（M10 scaffold --ucuf 自動生成骨架）
 *
 ...
 *
 * Content Contract：hud-overlay-content（schemaId）
 *   - 必填：請參見 assets/resources/ui-spec/contracts/battle-hud-overlay-screen-content.schema.json
 */
...
const CONTRACT: ContentContractRef = {
    ...
    requiredFields: [], // TODO: 根據 battle-hud-overlay-screen-content.schema.json 填入必填欄位
};

@ccclass('BattleHudOverlayCompositePanel')
export class BattleHudOverlayCompositePanel extends CompositePanel {

    // ── Screen ID（對應 screen spec 的 id，由 mount() 傳入）──────────────────
    private static readonly SCREEN_ID = 'battle-hud-overlay-screen';
    ...
    public async show(data: Record<string, unknown>): Promise<void> {
        if (!this._isMounted) {
            await this.mount(BattleHudOverlayCompositePanel.SCREEN_ID);
    ...
    protected override _onAfterBuildReady(_binder: UITemplateBinder): void {
        ...
        // btnClose?.node.on('click', () => services().ui.close('battle-hud-overlay-screen'), this);

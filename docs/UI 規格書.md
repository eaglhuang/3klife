...
> 2. `命 / Bloodline` 頁顯示英靈卡命槽、主卡切換、共鳴光效、14 人祖先血統圖、命紋靈獸、五階血脈傳承奧義與世家入口；底部故事條採可上拉 / 可縮回的 `StoryDock`。
...
- `日常人物頁`
  - 人物為主角
  - 屬性、戰鬥定位、血脈摘要可快速閱讀
- `命` 頁底部可固定一條可抽拉的 5~6 格故事條 `StoryDock`
- `血脈命鏡頁`
...
- `日常人物頁` 正式母型為：左人物、右主資訊；完整 5~6 格故事條優先落在 `命` 頁底部 `StoryDock`；血脈元素是深層人格區，不是第二主角
...
- `命` 頁的底部故事條必須採抽拉式 `StoryDock`，展開時不得直接覆蓋 14 人血統圖；應透過 reflow / split resize 讓血統圖與故事條並存
- `命紋靈獸` 在人物頁不是孤立圖騰，而是 `血脈傳承奧義` 的視覺核心；玩家應能從靈獸徽記、性格標籤與覺醒傾向直覺理解這條血脈會長成什麼能力

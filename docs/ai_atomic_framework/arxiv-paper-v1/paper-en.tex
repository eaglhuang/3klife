...
Operational overhead is likewise not quantified in this paper. ATM's deterministic gates do not themselves necessarily incur LLM token cost, but recovery routing after conservative admission can add measurable cost through queue wait time, serialization, rebase/replay, steward review, virtual-atom refinement, the composer path, neutral-steward apply, CAS recheck, and downstream validators. This paper does not claim latency, throughput, token-efficiency, memory-footprint, or starvation-freedom superiority. A future benchmark should therefore report at least P50/P95/P99 admission-decision latency, steward-apply latency, queue wait time, CAS-mismatch frequency, rebase/replay success rate, preserved-intent salvage rate, terminal fail-closed rate, over-serialization rate, full-regeneration rate, token rework cost when regeneration is actually required, and throughput under increasing numbers of concurrent agents.

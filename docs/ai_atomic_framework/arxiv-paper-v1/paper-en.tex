...
\begin{tikzpicture}[
  ...
  planelbl/.style={font=\tiny\bfseries, anchor=south west, inner sep=1pt},
  % Label variants — fill matches the plane backdrop so labels visually
  % merge with their plane instead of looking like white stickers glued on top.
  lbl/.style   ={font=\tiny\itshape, fill=white, fill opacity=0.94,
                 text opacity=1, rounded corners=1pt, inner sep=1.8pt},
  lblma/.style ={lbl, fill=green!9},
  lbltc/.style ={lbl, fill=blue!8},
  lblec/.style ={lbl, fill=orange!10}
]
% --- Task-contract plane (top) ---
\node[ext] (HU) at (-7.5, 3.0) {Human / Coordinator};
\node[tcplane, minimum width=120mm, minimum height=14mm] (TC) at (0.5, 3.0) {};
\node[pnode, fill=blue!22] (T)  at (-2.5, 3.0) {Task Contract\\\tiny$\langle g, A, F, S, D, V, E, \epsilon\rangle$};
\node[pnode, fill=blue!22] (DL) at ( 3.5, 3.0) {Direction Lock +\\Pre-tool Scope Gate};
\node[planelbl, blue!60!black] at (TC.north west) [yshift=1pt] {Task-contract plane};
% --- Mutation-admission plane (middle) ---
\node[maplane, minimum width=145mm, minimum height=34mm] (MA) at (0.5, -0.4) {};
\node[planelbl, green!40!black] at (MA.north west) [yshift=1pt]
      {Mutation-admission plane \,/\, CID broker subsystem};
\node[pnode, fill=green!22] (AG)   at (-5.5,  0.7)  {AI Agent\\\tiny WriteIntent};
\node[pnode, fill=green!22] (ADAP) at (-5.5, -1.5)  {Adapter Layer\\\tiny discoverAtomCandidates};
\node[pnode, fill=green!22] (AM)   at (-1.4,  0.7)  {Atom Map\\\tiny owners / deps / CID};
\node[pnode, fill=green!22] (VA)   at (-1.4, -1.5)  {Virtual Atoms\\\tiny refinement};
...
% --- Evidence-closure plane (bottom) ---
\node[ecplane, minimum width=120mm, minimum height=14mm] (EC) at (0.5, -4.2) {};
\node[planelbl, orange!60!black] at (EC.north west) [yshift=1pt] {Evidence-closure plane};
\node[pnode, fill=orange!22] (VE) at (-3.5, -4.2) {Validator Envelope};
\node[pnode, fill=orange!22] (EB) at ( 0.5, -4.2) {Evidence Blockers +\\Review Advisory};
\node[pnode, fill=orange!22] (CP) at ( 4.5, -4.2) {Closure Packet};
% --- Main governance path (solid) ---
\draw[flow] (HU)      -- (T);
\draw[flow] (T)       -- (DL);
% DL feeds AG: route up-and-over to keep clear of the MA plane border
\draw[flow] (DL.north) -- ++(0,0.55) -| (AG.north);
\draw[flow] (ADAP)    -- (AM);
\draw[flow] (AM)      -- (VA);
\draw[flow] (AM)      -- (AG);
% AG -> B: curve below AM, above VA, so it no longer crosses AM-VA link
\draw[flow] (AG.east) .. controls (-2.5, 0.25) and (1.0, 0.0) .. (B.west);
\draw[flow] (VA.east) .. controls ( 0.2,-1.4) and (1.6,-0.8) .. (B.south west);
\draw[flow] (AM.east) -- (B.north west);
\draw[flow] (B)       -- (ST);
\node[lblma] at ($(B.east)!0.52!(ST.west)+(0,0.28)$) {verdict + plan};
\draw[flow] (ST.south) -- ++(0,-1.4) -| (CP.north);
\draw[flow] (VE)      -- (EB);
\draw[flow] (EB)      -- (CP);
% --- Non-blocking dashed feedback channels (caption (a),(b),(c)) ---
% (a) evidence feedback: substrate -> broker
\draw[dashflow] (EB.east) -- ++(0.55,0) |- (B.south east);
\node[lblma, anchor=west] at ($(B.south east)+(0.22,-0.20)$) {evidence feedback};
% (b) post-write validators: validator envelope -> agent
\draw[dashflow] (VE.west) -- ++(-0.85,0) |- (ADAP.west);
\node[lblma, anchor=west] at ($(ADAP.west)+(-0.05,-0.34)$) {post-write validators};
% (c) epoch / direction update: closure packet -> task contract.
% Label placed midway on the long vertical segment so it does not collide
% with the Human/Coordinator box at the top-left.
\draw[dashflow] (CP.south) -- ++(0,-0.55) -| ([xshift=-3mm]HU.south) |- (T.west);
\node[lbl, anchor=west] at ($(EC.south)+(0.1,-0.46)$) {epoch update on scope change};
\end{tikzpicture}
\end{figure}
...
Algorithm 1 safety note. Physical disjointness remains necessary but not sufficient. It may confirm \texttt{\seqsplit{parallel-safe}} only when the intents are also outside the same physical file or structured artifact, and after shared-surface and read/write-dependency checks have cleared. Same-file bounded-disjoint edits are not treated as direct \texttt{\seqsplit{parallel-safe}} writes; they are routed to \texttt{\seqsplit{needs-physical-split}}, where the deterministic composer and the neutral steward produce a single governed apply path. A blocked or fail-closed branch is therefore a containment verdict over the direct-apply channel, not a default deletion of the proposing agent's work. This aligns Algorithm 1 with Figure 2, Table 2, the seven-layer gate, and the POS2 field case.
...
\node[field] (B4) at (-4.4, -4.0) {Field collision\\\tiny §4.4 \,/\, same-file\\\tiny POS2 / B-12 / BLOCK};
...
\node[wave]  (B6) at ( 7.4, -4.0) {Wave / Batch\\\tiny §4.5 \,/\, extension\\\tiny orchestration};
...
\end{scope}
\node[grouplbl, red!55!black]  at (GK.north west) [yshift=1pt] {admission evidence \,(supports K)};
\node[grouplbl, blue!55!black] at (GG.north west) [yshift=1pt] {governance evidence \,(supports G)};
...
\midrule
Framework mainline support & Repo-local broker evidence capture and parameterized evidence-path capability have landed in ATM mainline & Paper-evidence fast-path bundle \texttt{\seqsplit{191fd310166054d3fa526b6961351716bb2d489e}}; git-head backfill commit for this evidence freeze \texttt{\seqsplit{f57dbfe0bdfdf9f939e35400ec346501f4ccb2f3}} & Supports framework capability only; not an external public-source snapshot or live-conflict result \\
Phase A: FastAPI public-source snapshot governance & ATM can exercise a provenance-pinned FastAPI public-source snapshot inside a live host repo and preserve baseline, readiness, touched-path, command-log, and replay evidence & \texttt{\seqsplit{artifacts/external-public-repo/fastapi/2026-06-27/summary.json}}, \texttt{\seqsplit{paper-safe-summary.md}}, \texttt{\seqsplit{commands.log}}; upstream snapshot head \texttt{\seqsplit{82064857539e6286522c347b4b11331b48dd2378}}; host head \texttt{\seqsplit{738b9883880742cd36b64f1f81ce6a638f073135}} & Does not claim upstream FastAPI maintainer workflow governance; does not claim clean post-change snapshot-helper execution \\
Phase B: Structured Artifact Admission Track & ATM has deterministic cross-format admission and routing evidence for structured non-code artifacts & \texttt{\seqsplit{artifacts/generated/structured-artifact-admission/20260627-phase-b/summary.json}}, \texttt{\seqsplit{paper-safe-summary.md}}, \texttt{\seqsplit{results.jsonl}}, and \texttt{\seqsplit{docs/reports/structured-artifact-admission-track-2026-06-27.md}}; 15 / 15 matched cases across JSON, YAML, TOML, OpenAPI, and atom-map shard families & Does not claim external public-source snapshot governance; does not claim dual-editor live conflict \\
Phase C: Dual-live public-source conflict demonstration & ATM broker activates under Team Broker mode when two live actors touch the same FastAPI public-source snapshot path and records an auditable applied / queued / conflict run & Host-side broker-run envelope \texttt{\seqsplit{.atm/history/evidence/broker-runs/6ea4e411-fa2b-426b-9c71-55bbdbeaa888.json}}; plan id \texttt{\seqsplit{batch-5c1fd53c988116ce}}; target \texttt{\seqsplit{local/public-source-snapshots/fastapi-0.136.3/fastapi/\_\_init\_\_.py}} & Re-verified host evidence for one live conflict route; does not claim all runtime races are solved and does not claim upstream repo governance \\
\bottomrule
\end{longtable}
\endgroup
...
\midrule
Core broker and admission path & Broker decision logic, seven-layer gate, ConflictKey behavior, CAS base-hash guarded apply, steward routing & \texttt{\seqsplit{npm test -- broker/decision}}; \texttt{\seqsplit{npm test -- broker/cas}}; \texttt{\seqsplit{npm test -- broker/steward}}; adapter benchmark commands listed in the repository \\
Atom identity and adapter support & Atom-to-CID validation, TypeScript/Python reference paths, format adapters, virtual-atom refinement path & \texttt{\seqsplit{node --strip-types scripts/validate-atom-id-to-cid.ts}}; \texttt{\seqsplit{node --strip-types scripts/validate-agr-benchmark.ts}}; adapter self-hosting benchmark \\
AdmissionBench result chain & v0.1 baseline, blind export, v0.2 paper profile, report rendering, and audit replay & \texttt{\seqsplit{npm run bench:admission:smoke -- --seed 20260625}}; \texttt{\seqsplit{npm run bench:admission:paper -- --seed 20260625}}; \texttt{\seqsplit{npm run bench:admission:report -- --seed 20260625}} \\
OperationalBench result chain & Official runtime-overhead run, N=50 extended run, multi-seed stability, and validation of operational artifacts & \texttt{\seqsplit{npm run bench:operational:paper -- --seed 20260627}}; extended and multi-seed commands listed in supplementary verification notes \\
...
Topology C pre-push bridge & Local pre-push admission bridge, dry-run path, push-fail fallback, and Git-to-broker \texttt{\seqsplit{MutationRequest}} construction & \texttt{\seqsplit{atm git admit --dry-run}}; Topology C plan document and archived bridge artifacts \\
\bottomrule
\end{longtable}
\endgroup
...
\item For OperationalBench benchmark evidence: \texttt{\seqsplit{AI-Atomic-Framework, main commit c0250009a53b28e887344e71ea675637c97290b0, artifacts/generated/atm-operational-bench/20260627/, 20260627-extended/, and multi-seed-stability-20260627-20260629.\{md,json\}. https://github.com/eaglhuang/AI-Atomic-Framework}}
\end{itemize}
...
\begin{enumerate}
\item Pugachev, Sergey. 2025. {\char34}CodeCRDT: Observation-Driven Coordination for Multi-Agent LLM Code Generation.{\char34} arXiv:2510.18893 [cs.DC]. \url{https://doi.org/10.48550/arXiv.2510.18893}.
\item Acharya, Vivek. 2026. {\char34}Semantic Consensus: Process-Aware Conflict Detection and Resolution for Enterprise Multi-Agent LLM Systems.{\char34} arXiv:2604.16339 [cs.AI]. \url{https://doi.org/10.48550/arXiv.2604.16339}.
\item Liu, Mengyang, Taozhi Chen, Zhenhua Xu, Xue Jiang, and Yihong Dong. 2026. {\char34}Multi-agent Collaboration with State Management.{\char34} arXiv:2605.20563 [cs.MA]. \url{https://doi.org/10.48550/arXiv.2605.20563}.
\item Qian, Kaiyang, Xinmin Fang, and Zhengxiong Li. 2026. {\char34}MPAC: A Multi-Principal Agent Coordination Protocol for Interoperable Multi-Agent Collaboration.{\char34} arXiv:2604.09744 [cs.MA]. \url{https://doi.org/10.48550/arXiv.2604.09744}.
\item Zhou, Weixing, Zhiyou Wang, Zeshun Peng, Hetian Chen, Yanfeng Zhang, and Ge Yu. 2026. {\char34}ATCC: Adaptive Concurrency Control for Unforeseen Agentic Transactions.{\char34} arXiv:2603.13906 [cs.DB]. \url{https://doi.org/10.48550/arXiv.2603.13906}.
...
\item Sartori, Camilo Chacon. 2026. {\char34}The Specification Gap: Coordination Failure Under Partial Knowledge in Code Agents.{\char34} arXiv:2603.24284 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2603.24284}.
\item Ellis, Clarence A., and Simon J. Gibbs. 1989. {\char34}Concurrency Control in Groupware Systems.{\char34} In *Proceedings of the 1989 ACM SIGMOD International Conference on Management of Data*, 399-407. New York: ACM Press. \url{https://doi.org/10.1145/67544.66963}.
...
\item Kung, H. T., and John T. Robinson. 1981. {\char34}On Optimistic Methods for Concurrency Control.{\char34} *ACM Transactions on Database Systems* 6 (2): 213-226. \url{https://doi.org/10.1145/319566.319567}.
\item Lyu, Hongtao, Dingyan Zhang, Mingyu Wu, Xingda Wei, and Haibo Chen. 2026. {\char34}CoAgent: Concurrency Control for Multi-Agent Systems.{\char34} arXiv:2606.15376 [cs.DC]. \url{https://doi.org/10.48550/arXiv.2606.15376}.
\item Geng, Jiayi, and Graham Neubig. 2026. {\char34}Effective Strategies for Asynchronous Software Engineering Agents.{\char34} arXiv:2603.21489 [cs.CL]. \url{https://doi.org/10.48550/arXiv.2603.21489}.
\item Zhang, Qingyu, Junzhe Li, Jiayi Lin, Changhua Luo, and Chenxiong Qian. 2026. {\char34}Rover: Context-aware Conflict Resolution with LLM.{\char34} arXiv:2605.17279 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2605.17279}.
\item Ogenrwot, Daniel, and John Businge. 2026. {\char34}AgenticFlict: A Large-Scale Dataset of Merge Conflicts in AI Coding Agent Pull Requests on GitHub.{\char34} arXiv:2604.03551 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2604.03551}.
\item Wang, Yifei, Ruiyin Li, Peng Liang, Qiong Feng, Zengyang Li, Mojtaba Shahin, and Arif Ali Khan. 2026. {\char34}CodeTeam: An LLM-Powered Multi-Agent Framework for Repository-Level Code Generation.{\char34} arXiv:2606.22082 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2606.22082}.
\item Khan, Sajjad. 2026. {\char34}S-Bus: Automatic Read-Set Reconstruction for Multi-Agent LLM State Coordination.{\char34} arXiv:2605.17076 [cs.LG]. \url{https://doi.org/10.48550/arXiv.2605.17076}.
\item Huang, Beichen, Ran Cheng, and Kay Chen Tan. 2025. {\char34}EvoGit: Decentralized Code Evolution via Git-Based Multi-Agent Collaboration.{\char34} arXiv:2506.02049 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2506.02049}.
\item Li, Yang, Siqi Ping, Xiyu Chen, Xiaojian Qi, Zigan Wang, Ye Luo, and Xiaowei Zhang. 2025. {\char34}AgentGit: A Version Control Framework for Reliable and Scalable LLM-Powered Multi-Agent Systems.{\char34} arXiv:2511.00628 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2511.00628}.
\item Jimenez, Carlos E., John Yang, Alexander Wettig, Shunyu Yao, Kexin Pei, Ofir Press, and Karthik Narasimhan. 2023. {\char34}SWE-bench: Can Language Models Resolve Real-World GitHub Issues?{\char34} arXiv:2310.06770 [cs.CL]. \url{https://doi.org/10.48550/arXiv.2310.06770}.
...
\item Adya, Atul. 1999. {\char34}Weak Consistency: A Generalized Theory and Optimistic Implementations for Distributed Transactions.{\char34} PhD thesis, Massachusetts Institute of Technology. \url{https://hdl.handle.net/1721.1/149899}.
\item Lloyd, Wyatt, Michael J. Freedman, Michael Kaminsky, and David G. Andersen. 2011. {\char34}Don't Settle for Eventual: Scalable Causal Consistency for Wide-Area Storage with COPS.{\char34} In *Proceedings of the 23rd ACM Symposium on Operating Systems Principles*, 401-416. \url{https://doi.org/10.1145/2043556.2043593}.
\item Liu, Tianyang, Canwen Xu, and Julian McAuley. 2024. {\char34}RepoBench: Benchmarking Repository-Level Code Auto-Completion Systems.{\char34} In *Proceedings of the 12th International Conference on Learning Representations (ICLR)*. \url{https://doi.org/10.48550/arXiv.2306.03091}.
...
\item Sun, Chengzheng, Xiaohua Jia, Yanchun Zhang, Yun Yang, and David Chen. 1998. {\char34}Achieving Convergence, Causality Preservation, and Intention Preservation in Real-Time Cooperative Editing Systems.{\char34} *ACM Transactions on Computer-Human Interaction* 5 (1): 63-108. \url{https://doi.org/10.1145/274444.274447}.
\item Chacon, Scott, and Ben Straub. 2014. *Pro Git*, 2nd ed. Apress / Open Source. \url{https://git-scm.com/book}.
\item Bernstein, Philip A., Vassos Hadzilacos, and Nathan Goodman. 1987. *Concurrency Control and Recovery in Database Systems*. Reading, MA: Addison-Wesley. \url{https://www.microsoft.com/en-us/research/people/philbe/book/}.
...
\item Zhao, Wenting, Nan Jiang, Celine Lee, Justin T. Chiu, Claire Cardie, Matthias Gallé, and Alexander M. Rush. 2025. {\char34}Commit0: Library Generation from Scratch.{\char34} In *Proceedings of the 13th International Conference on Learning Representations (ICLR)*. arXiv:2412.01769 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2412.01769}.
...
\item Wang, Haoyu, Christopher M. Poskitt, and Jun Sun. 2026. {\char34}AgentSpec: Customizable Runtime Enforcement for Safe and Reliable LLM Agents.{\char34} In *Proceedings of the 48th IEEE/ACM International Conference on Software Engineering (ICSE 2026)*. \url{https://doi.org/10.1145/3744916.3764546}.
\item Zhao, Wei, Zhe Li, Peixin Zhang, and Jun Sun. 2026. {\char34}ClawGuard: A Runtime Security Framework for Tool-Augmented LLM Agents Against Indirect Prompt Injection.{\char34} arXiv:2604.11790 [cs.CR]. \url{https://doi.org/10.48550/arXiv.2604.11790}.
\item Winston, Cailin, Claris Winston, and René Just. 2026. {\char34}Solver-Aided Verification of Policy Compliance in Tool-Augmented LLM Agents.{\char34} arXiv:2603.20449 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2603.20449}.
\item Sousa, Marcelo, Isil Dillig, and Shuvendu K. Lahiri. 2018. {\char34}Verifying Semantic Conflict-Freedom in Three-Way Program Merges.{\char34} arXiv:1802.06551 [cs.PL]. \url{https://doi.org/10.48550/arXiv.1802.06551}.
\item Cavalcanti, Guilherme, Paulo Borba, Leonardo dos Anjos, and Jonatas Clementino. 2024. {\char34}Semistructured Merge with Language-Specific Syntactic Separators.{\char34} arXiv:2407.18888 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2407.18888}.
\item Mohammadi, Bardia, Nearchos Potamitis, Lars Klein, Akhil Arora, and Laurent Bindschaedler. 2026. {\char34}Atomix: Timely, Transactional Tool Use for Reliable Agentic Workflows.{\char34} arXiv:2602.14849 [cs.LG]. \url{https://doi.org/10.48550/arXiv.2602.14849}.
\item Chen, Zheng, Hanqing Liu, Duling Xu, Dong Dong, Jialin Li, Bangzheng Pu, and Jidong Zhai. 2026. {\char34}Cordon: Semantic Transactions for Tool-Using LLM Agents.{\char34} arXiv:2606.17573 [cs.OS]. \url{https://doi.org/10.48550/arXiv.2606.17573}.
\item Guo, Xuehang, Xingyao Wang, Yangyi Chen, Sha Li, Chi Han, Manling Li, and Heng Ji. 2025. {\char34}SyncMind: Measuring Agent Out-of-Sync Recovery in Collaborative Software Engineering.{\char34} arXiv:2502.06994 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2502.06994}.
\item Mao, Zhenyu, Jacky Keung, Fengji Zhang, Shuo Liu, Yifei Wang, and Jialong Li. 2025. {\char34}Towards Engineering Multi-Agent LLMs: A Protocol-Driven Approach.{\char34} arXiv:2510.12120 [cs.SE]. \url{https://doi.org/10.48550/arXiv.2510.12120}.
\item Hou, Bo, Xin Tan, Kai Zheng, Fang Liu, Yinghao Zhu, and Li Zhang. 2025. {\char34}LLM-Driven Collaborative Model for Untangling Commits via Explicit and Implicit Dependency Reasoning.{\char34} arXiv:2507.16395 [cs.AI]. \url{https://doi.org/10.48550/arXiv.2507.16395}.
\end{enumerate}

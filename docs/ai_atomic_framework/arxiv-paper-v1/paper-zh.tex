...
本文將 ATM 定位為 \textbf{specification-to-evidence governance substrate}，而非單純 concurrency broker：它把 task intent、repository scope、write admission、validator 與 evidence obligation 結合為一條單域治理鏈；CID broker 僅是其中的共享變更准入子系統，用於治理 shared repository mutation 的進入權。核心流程為 adapter-guided atomization 將 intent 映射成 semantic atoms，經 CID-brokered admission 做並行／序列化／阻擋判斷；當 atom map 覆蓋不足時，virtual atom 作為過渡治理單位保留 bounded-region 比較能力；實際寫入則由同一治理域內的 neutral steward 執行，避免多個代理直接在共享檔案直接落筆。

本文以「五條 evidence strands」支持上述主張： （i）12-scenario deterministic fixture 與三個 archived runner cases（decision surface）； （ii）三週外部採用者治理觀察（adopter-side recoverability）； （iii）一條完整歸檔的 POS2 正向、兩類負向案例（admission-time 漏抓、apply-time fail-closed、admission-time 直接阻擋）； （iv）批次排程與 CID identity 穩定性延伸觀察； （v）由 \textbf{v0.1 baseline} 與 \textbf{v0.2 paper profile} 共同組成的 ATM-AdmissionBench：v0.1 提供凍結 benchmark/substrate 與盲審邊界，v0.2 提供論文結果層（20-scenario / 42-comparison、0 expectation failures、0 unresolved rows、ATM-full route F1 = 1.000，並有 252 policy rows、294 ablation rows、210 adversarial rows、4 enforcement rows）。 這五條證據支持「單一治理域內的可行性」與「有界 recoverability」，但不構成對其他併發控制系統的全面比較性優劣結論。ATM 不取代 Git merge，也不涵蓋跨 clone 或跨 PR 的分散式治理。核心主張限定於：在單一受控治理域內，ATM 提供可實作、可審計且可逐步擴充的入場前準入層（可由 \texttt{\seqsplit{generator-manifest.json}}、\texttt{\seqsplit{summary.json}} 與 \texttt{\seqsplit{paper-tables.md}} 交叉查核）。

\section{1. Introduction（緒論）}
...
\section{4. Validation, Evidence, and Benchmark Alignment（驗證、證據與基準對齊）}
...
\noindent\textbf{Table 11 --- Experiment Status Map. 本表整理本文所使用或明確排除的 deterministic、field replay、adoption summary 與 comparative experiments。狀態欄區分 archived、replayable、summary-only、deferred 與 out-of-scope，以維持 evidence claim 與可重現程度之間的一致性。}
...
\midrule
MAO-0010 parallel routing benchmark replay & deterministic simulator & 12 scenarios，offline 確定性 & archived + replayable（\texttt{\seqsplit{docs/reports/mao-parallel-routing-benchmark.md}}） & broker admission vocabulary 對齊 \\
ATM-AdmissionBench v0.1 baseline + v0.2 paper profile & deterministic benchmark substrate + paper-facing result & 20 scenarios / 42 comparisons；v0.1 提供 virtual-atom admission pack + conflict-arbitration pack；v0.2 補上 policy / ablation / adversarial / enforcement summary & v0.1 frozen generator commit \texttt{\seqsplit{3eec69a73a04112e2af8d3630c32138c37143eab}}; v0.2 aligned generator/base commit \texttt{\seqsplit{9a0c03e59d789c76e65f2b1186acb66979cc4e89}} & baseline audit anchor + paper-facing reporting anchor \\
...
POS2 same-owner bounded admission replay & controlled field & 同檔 disjoint bounded region；雙 vendor 重放模板 \texttt{\seqsplit{bench-paper-hotfile-pos2-\{a,b\}-intent.json}} & archived + template-replayable & same-file admission positive boundary \\
...
\texttt{\seqsplit{parallel-0041-0042}} 跨 vendor real-task collision replay & field & 5 共同檔；Cursor Composer 2.5 + Google Gemini Flash 3.5；broker$\rightarrow$wave 序列化路徑 & archived（\texttt{\seqsplit{CID-Conflict-Run-Log.md}}：\texttt{\seqsplit{67b193f9}} / \texttt{\seqsplit{c393df1d}}） & multi-vendor field collision governance path \\
...
STORM / CodeCRDT / SCF / CoAgent 對照式 throughput benchmark & comparative & end-to-end conflict catch rate / wall-clock / token cost & \textbf{deferred to future work}；本版不承諾 & §5 limitations \\
AgenticFlict 重放 ATM broker（142K agent PR / 27.67\% conflict / 336K+ region） & comparative replay & 跨 PR / Git merge sample $\rightarrow$ 單一治理域寫入前 intent 重放工作負載 & \textbf{deferred}，需先建立轉換層；本版不執行 & §5 limitations / motivation only \\
跨 clone 分散式 admission 模擬 & distributed & 多 worktree / 多 clone 場景 & \textbf{out of scope}（§3.7, §5）；非本文 deliverable & §3.7 explicit non-goal \\
\bottomrule
\end{longtable}
\endgroup
...
本節提供的是 \textbf{self-hosting governance evidence}：它不主張外部效度，而是用來說明 ATM 在治理自身 framework 與論文工件時，是否已具備可追溯的治理基礎與可回饋的缺口揭露能力。Table 13 先給出最小但可追溯的 self-hosting 覆蓋概況，避免將 ATM 自我治理誤解為少數示範案例。這些指標不等同於宣稱所有治理 surface 均已完成細粒度原子化；它們要表達的是，ATM 對自身的 atomization governance 已具備可量化的系統性基礎。

\noindent\textbf{Table 13 --- Self-Hosting Atomization Governance Coverage.}
...
\noindent\textbf{Table 14 --- Self-Hosting Incident Types.}
...
為了避免讀者誤以為這三類事件就是 ATM 自宿觀察的全部 collision，本文補上 self-hosting window 的工作量基線。AAF 框架本體自 2026-05-01 起的觀察窗口內累積約 1,372 個 governed commits；同期 3KLife 在 2026-05-19 至 2026-06-07 三週 npc-brain 對齊窗口中累積 320 個 governed commits，並由 15 個不同 vendor / editor channel 的 AI agent（含 \texttt{\seqsplit{claude-code-*}}、\texttt{\seqsplit{codex-gpt-5.*}}、\texttt{\seqsplit{antigravity-gemini-3.5-flash}}、\texttt{\seqsplit{vs-code-copilot-agent}}、\texttt{\seqsplit{vs-insiders-gpt-5.4}} 等變體）共同寫入。這是一個高頻、多供應商、同治理域的 workload；Table 14 三類事件即在此 workload 下被反覆觸發、被機制吸收、被導向 freeze / patch-envelope / waiver / mechanized closeout。本文無法宣稱所有 collision 都已被計入 incident ledger，但可說明：在此 \textasciitilde{}1,700 commits 等級的自宿樣本中，「無法被治理層吸收」的事件不是少數展示案例，而是已被機制化處理的常態；其證據強度因此高於 anecdote，但不等同於對照基準。
...
第四是 \textbf{conclusion validity}。本文提供的是帶有結果層級區分的 benchmark chain，而\textbf{非}完整大規模對照式 benchmark：v0.1 承擔 baseline 與 audit anchor，v0.2 承擔 paper-facing result；兩者合起來已足以支持 route F1、layer ablation 與 enforcement timing 的正文敘事，但本文仍未報告 controlled comparative benchmark、confidence intervals 或 power analyses。因此，本文較適合支持「ATM 在其宣稱邊界內具可行性、可審核性與方法新穎性」之結論，而\textbf{不足以}對 throughput、false-positive rate、token efficiency 或跨系統優劣做統計性定論；後者屬 §5.3 與 §6.3 所列之後續 comparative evaluation 範圍。
...
\section{5. AdmissionBench Results and Limitations（AdmissionBench 結果與限制）}
...
RQ3: Routing correctness & ATM 是否能選擇 \texttt{\seqsplit{parallel}}、\texttt{\seqsplit{compose}}、\texttt{\seqsplit{serial}}、\texttt{\seqsplit{block}} 或 \texttt{\seqsplit{re-arbitrate}} 類路由？ & v0.1 baseline 已在 42 / 42 expected verdict comparisons 上全數吻合；v0.2 進一步報告 ATM-full route F1 = 1.000。 & 擴展到更大 scenario family 與更多 adapter 組合。 \\
...
\subsubsection*{6.4.1 Topology A --- 單一 workstation 多 vendor 共寫（本文 field evidence 的核心部署拓樸）}
...
\subsubsection*{6.4.2 Topology B --- 共享地端伺服器多 vendor AI 共寫，遠端 human prompt 輸入（deployment-only extension；ATM 軟體本身無需改動）}
...
\subsubsection*{6.4.3 Topology C --- 本地 pre-push admission bridge（已完成內部驗證）}
...
\subsubsection*{6.4.4 三 topology 之共同假設與分工}
...
\subsubsection*{6.4.5 更遙遠之延伸：Topology D 跨機器 patch 同步（out-of-scope distributed extension）}
...
\section*{A.1 Evidence Artifact Map}
...
本節先固定一個最重要的引用規則：\textbf{對本文正文數字與 paper-facing benchmark claims 的 canonical public anchor，預設以 AAF \texttt{\seqsplit{main@ab8753b7daf0a3c4cd8b4483fe24d519ff2590bd}} 下的 \texttt{\seqsplit{artifacts/generated/atm-admission-bench/20260625-paper/}} 為主}。\texttt{\seqsplit{v0.9.0-alpha.1}} source snapshot 與 \texttt{\seqsplit{generator-manifest.json}} 內記錄的 machine-generated source anchor 仍然重要，但角色分別是 source-reference snapshot 與 generator provenance anchor，不應與 paper-facing artifact anchor 混讀為同一層引用點。

\begin{itemize}
\item \textbf{公開部分（public）：} ATM 框架本體之 source、deterministic runner MVP、12-scenario fixture design、ATM-AdmissionBench v0.1 baseline artifacts、ATM-AdmissionBench v0.2 paper-profile artifacts、broker decision implementation、CID validation scripts、Topology C bridge 與多數 self-hosting governance reports，存放於 GitHub 公開 repository \texttt{\seqsplit{eaglhuang/AI-Atomic-Framework}}。本論文之 source reference snapshot 為 release tag \texttt{\seqsplit{v0.9.0-alpha.1}} 對應 commit \texttt{\seqsplit{0b31aa8683b44b3a78206132a0bf90a0fde73d1c}}；AdmissionBench v0.1 baseline 與 v0.2 paper profile 的 machine-generated source anchor 均為 \texttt{\seqsplit{generator-manifest.json}} 所記錄之 \texttt{\seqsplit{3eec69a73a04112e2af8d3630c32138c37143eab}}。若審稿者需要逐項核對本文數字，應以 AAF \texttt{\seqsplit{main@ab8753b7daf0a3c4cd8b4483fe24d519ff2590bd}} 下的 \texttt{\seqsplit{artifacts/generated/atm-admission-bench/20260625-paper/}} 作為第一入口，再用其中的 \texttt{\seqsplit{generator-manifest.json}}、\texttt{\seqsplit{summary.json}}、\texttt{\seqsplit{paper-tables.md}}、\texttt{\seqsplit{artifact-hash-manifest.sha256}} 與對應的 \texttt{\seqsplit{20260625/}} baseline 目錄交叉查對。
...
\midrule
12-scenario suite design & controlled decision-surface coverage blueprint & \texttt{\seqsplit{docs/ai\_atomic\_framework/arxiv-paper-v1/bench-design.md}} & public（AAF repo） \\
deterministic runner MVP & archived synthetic mechanism evidence & \texttt{\seqsplit{tools/multi-vendor-broker-bench/README.md}} & public（AAF repo） \\
ATM-AdmissionBench v0.1 baseline + v0.2 paper profile & machine-generated baseline summary, label-retained blind package, paper-facing summary tables, provenance manifest, audit findings & canonical public anchor: \texttt{\seqsplit{main@ab8753b7daf0a3c4cd8b4483fe24d519ff2590bd}} $\rightarrow$ \texttt{\seqsplit{artifacts/generated/atm-admission-bench/20260625-paper/}}; secondary anchors: \texttt{\seqsplit{artifacts/generated/atm-admission-bench/20260625/}}, \texttt{\seqsplit{artifacts/blind-bench/20260625/}}, \texttt{\seqsplit{docs/reviews/ATM-AdmissionBench-audit.md}} & public（AAF repo / supplementary release）；v0.1 作為 baseline anchor，v0.2 作為 paper-facing reporting anchor \\
...
Wave Mode / CID stability & orchestration extension and identity substrate evidence & \texttt{\seqsplit{docs/reports/team-wave-mode-validation.md}} 與相關 CID validation scripts & public（AAF repo） \\
\bottomrule
\end{longtable}
\endgroup

\section*{A.2 Implementation / Commit Provenance}
...
\noindent\textbf{Table A.2 --- Implementation Status and Evidence Role.}
...
\section*{A.3 CID Schema Migration Candidate Paths}
...
\section*{A.4 Implementation Verification Map and Topology C Bridge Detail}
...
\subsection*{A.4.1 Verification Map}
...
\subsection*{A.4.2 Topology C MVP Mechanics（internal validation evidence）}
...
\subsection*{A.4.3 Topology C Implementation Stages（2026-06-26 完成）}
...
\subsection*{A.4.4 Artifact Manifest Snapshot}
...
\texttt{\seqsplit{artifacts/generated/atm-admission-bench/20260625-paper/}} & AAF evidence commit \texttt{\seqsplit{ab8753b7daf0a3c4cd8b4483fe24d519ff2590bd}} & v0.2 paper-profile result bundle & public release bundle \\
...
\subsection*{A.4.5 Topology C Non-Goals (MVP)}
...
\subsection*{A.4.6 Topology C Acceptance Conditions}
...
\section*{A.5 Condensed Incident Table}

下表將 TASK-CID-0040\textasciitilde{}0045 的 incident evidence 濃縮為三類治理失敗模式；詳細 closure packet、repair commit 與 forensic report 路徑仍以 A.1 與原始 incident archive 為準。

\noindent\textbf{Table A.5 --- Condensed Incident Evidence.}
...
\midrule
\texttt{\seqsplit{TASK-CID-0040}} claim-displaced-by-import & claim ledger consistency、in-progress claim collision detection & 匯入流程一度覆寫進行中的 claim，後續由 event-ledger mismatch 偵測並修復；此案說明 governance state 必須有可重放的 claim/repair 軌跡。 \\
\texttt{\seqsplit{TASK-CID-0041}} out-of-scope delivery requiring waiver & scope gate、closure packet waiver、late enforcement traceability & admission 未在寫入當下完整攔住 scope drift，而是由後續 closure packet 以明示 waiver 補登；此案支撐本文對 enforcement boundary 的誠實揭露，而非將所有案例包裝成正向成功。 \\
\texttt{\seqsplit{TASK-CID-0043}} / \texttt{\seqsplit{0044}} / \texttt{\seqsplit{0045}} plan-mirror sync failures & sole-serialization invariant、planning mirror / target ledger closeout consistency & planning 端與 target ledger 曾出現 closeout 漂移，需以 repair commits 回補 closure packets；此案說明 broker / steward 必須成為唯一正式 closeout authority。 \\
\bottomrule
\end{longtable}
\endgroup
...
Readers wishing to inspect the manuscript-side admission evidence are referred to the public AAF repository at commit \texttt{\seqsplit{0b31aa8683b44b3a78206132a0bf90a0fde73d1c}} (release tag \texttt{\seqsplit{v0.9.0-alpha.1}}) and to the supplementary data release described in Appendix A.1. Internal task ledgers, vendor account identifiers, and adopter-side records are not redistributed verbatim; de-identified evidence chains, verdict logs, and validator traces are made available on request, subject to the access conditions stated in Appendix A.1.
...
Anchor 類別 & 識別 & 狀態 & 用途 \\
\midrule
Source release tag（AAF） & \texttt{\seqsplit{v0.9.0-alpha.1}} & \textbf{published} & 本論文所引 source path 與行號之 frozen reference；對應 commit \texttt{\seqsplit{0b31aa8683b44b3a78206132a0bf90a0fde73d1c}} \\
...
(vi) \textbf{multi-vendor self-hosting window summary}：§4.5 提及之同窗口跨 vendor agent 寫入統計（AAF 1,372 commits、3KLife 320 commits / 15 channels）之 commit-author histogram 與時間序列摘要，不含 commit message 內容。

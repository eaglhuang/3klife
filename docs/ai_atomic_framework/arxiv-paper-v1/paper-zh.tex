...
\author{Eagl Huang\\Independent Research}
\date{2026-06-26}
...
Reproducibility：本論文所有主張為「已實作且可重現」之 capability，均對應 AI-Atomic-Framework 開源 repository（\texttt{\seqsplit{https://github.com/eaglhuang/AI-Atomic-Framework}}）中的既有 source path 與可重現驗證命令。本文交叉檢查所對應的 framework snapshot 為 release tag \texttt{\seqsplit{v0.9.0-alpha.1}}（commit \texttt{\seqsplit{0b31aa8683b44b3a78206132a0bf90a0fde73d1c}}），讀者應以此 tag 作為主要引用點，避免 main 分支演進造成行號漂移。Appendix A.4 提供逐條 capability claim $\rightarrow$ source path $\rightarrow$ 可重現驗證命令之對應表，supplementary evidence artifact 集中於 3KLife planning repository 之 \texttt{\seqsplit{docs/ai\_atomic\_framework/broker-collision-evidence/}}。
...
\smallskip\noindent{\small\bfseries Table 1 --- Related-Work Citation-to-Claim Map.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 2 --- System Boundary Matrix.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 3 --- ATM-Distinct Capabilities for Pre-Write Admission.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 4 --- Tier Granularity Ladder. 本文於協調粒度階梯中的位置：}\par\smallskip
...
State drift & intent 基於已變動之 base / read dependency & active registry、\texttt{\seqsplit{readAtoms}}、CAS base-hash \\
\bottomrule
\end{longtable}
\endgroup
...
\smallskip\noindent{\small\bfseries Table 5 --- Governance Object Comparison.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 6 --- Broker Verdicts and Follow-Up Paths.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 7 --- Seven-Layer Admission Gate（含成熟度標記）。\textbf{成熟度欄位定義：} Proven 指已有歸檔 deterministic runner 證據或 archived field collision 證據支撐；Partial 指核心機制已實作並落地，但邊界情境尚未跑完完整 regression；Speculative 指框架已預留位置與最小落地，但完整自治路徑（如全自動 virtual-atom refinement、自治式 bounded-re-plan）仍屬演進中工作，本文不主張其等同於 admission core deliverable。}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 8 --- ConflictKey Generalization Matrix. Scope $\times$ Locator 跨格式映射：}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 9 --- Evidence Boundary Overview. 此表彙整 §4.1--§4.5 使用的主要證據來源、觀測量與支撐主張；最後兩欄明示其證據邊界與 artifact 來源，避免將 adoption、self-hosting 或 extension evidence 外推為 same-file admission 的直接證據。}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 10 --- Verdict Phase Map. 本表將 ATM 在證據材料中出現的 verdict、lane 與 state 對應到其發生相位。此表的用途是區分 admission-time decision、apply-phase arbitration 與 post-apply validation，而非將所有詞彙視為同一層級的 admission verdict。}\par\smallskip
...
重要前置說明： 本節的 evidence 強度分兩層 ------「12-scenario」指的是 \textbf{設計矩陣（design matrix）}，「3 archived」指的是 \textbf{已完成 deterministic runner 並歸檔證據} 的核心情境（B-02、B-08、B-13）。其餘 9 個情境屬於本版尚未完跑的設計覆蓋藍圖，列入 §5 limitations 與 future work，而非已驗證 deliverable。
...
\smallskip\noindent{\small\bfseries Table 12 --- Deterministic Fixture Coverage Categories.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 15 --- npc-brain Three-Week Adoption Summary.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 16 --- Governance-Containment Mapping: existing evidence $\times$ three planes.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 18 --- ATM-AdmissionBench v0.1 Baseline vs. v0.2 Paper Profile.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 19 --- ATM-AdmissionBench v0.2 Paper-Facing Summary.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table 20 --- AdmissionBench Research Questions and Current Evidence.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table A.1 --- Evidence Artifact Index.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table A.3 --- Verification Map.}\par\smallskip
...
\smallskip\noindent{\small\bfseries Table A.4 --- Topology C Implementation Stages.}\par\smallskip
...
版本對應約定： 本論文 v3.1 對應 source release \texttt{\seqsplit{v0.9.0-alpha.1}}；若後續 source repository 因主分支演進需發行新 release（例如 \texttt{\seqsplit{v0.9.0-alpha.2}}），本論文不會自動跟隨升版------任何更動需以 paper revision（v3.2 / v4 等）形式同步釋出，並在 Appendix C.2 補上新行；舊行不刪除，以維持引用鏈完整。

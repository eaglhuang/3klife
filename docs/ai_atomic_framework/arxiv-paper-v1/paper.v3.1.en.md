...
The related-work discussion below is organized as a citation-to-claim map, but the full map is moved to the supplementary artifact index to avoid turning the body into a bibliography table. The body retains only the comparison layers needed to locate ATM's intervention boundary. Repository-level benchmarks establish the multi-file and long-horizon task pressure; agentic concurrency-control systems such as CoAgent, S-Bus, and ATCC define adjacent shared-state coordination substrates; transactional tool-effect runtimes such as Cordon and Atomix define neighboring settlement layers; and repository-level workflow or protocol systems such as CodeTeam, SEMAP, MPAC, CodeCRDT, AgentGit, and EvoGit define adjacent planning, coordination, or convergence points. These systems are not treated as direct baselines for the present paper. ATM's claim remains narrower: repository-scoped pre-write admission for governed shared mutation inside a single governance domain.
...
The following table summarizes selected neighboring systems at the boundary most relevant to ATM. It neither ranks overall system quality nor assumes that the systems share identical research objectives. Table 1 identifies each system's primary coordination layer, intervention point, and downstream integration or apply path, which is the comparison surface needed in the body.

**Table 1 — Selected System Boundary Matrix.**
...
Table 1 is the body-level comparison table for related systems. It records where each system intervenes relative to repository mutation and whether it offers same-file bounded-region admission. A more detailed capability grid is provided in the supplementary citation-to-claim map. The body does not repeat that grid because the main comparison point is already captured by Table 1: ATM intervenes at the repository-region pre-write boundary, whereas neighboring systems operate at character-level convergence, file-level state mediation, workspace isolation, protocol-level coordination, transactional tool-effect settlement, or end-to-end repository generation. The table is descriptive rather than competitive; it does not imply that ATM replaces these systems, nor does it evaluate capabilities outside the pre-write admission boundary.
...
Adjacent work on content grounding, action-boundary enforcement, and solver-aided policy checking motivates ATM's evidence-closure and task-contract planes, but these systems do not directly enforce repository-scoped shared-mutation admission. SWE-agent shows that the agent-computer interface materially affects automated software-engineering performance. AgentSpec and ClawGuard show that structured runtime policies can constrain tool-using agents at action boundaries. Solver-aided policy checking points to a future direction in which a subset of ATM's forbidden rules and task-contract predicates could become machine-checkable. ATM adapts these boundary-enforcement ideas to a repository-governance setting by connecting task scope, mutation admission, validators, evidence obligations, and closure packets into one single-domain governance path.
...
**Table 2 — Governance Object Comparison.**
...
**Table 3 — Broker Verdicts and Follow-Up Paths.**
...
**Table 4 — Seven-Layer Admission Gate (with maturity markers).** The maturity column uses three labels:
...
**Table 5 — ConflictKey Generalization Matrix.** Scope × Locator mapping across formats.
...
**Table 6 — Deterministic Fixture Coverage Categories.**
...
**Table 7 — npc-brain Three-Week Adoption Summary.**
...
**Table 8 — Separated Evidence Lines for Public-Source and Structured-Artifact Governance.**
...
**Table 9 — Baseline Policy Definitions.**
...
**Table 11 — ATM-AdmissionBench v0.2 Paper-Facing Summary.**
...
**Table 12 — OperationalBench Latency Summary.**
...
**Table 13 — Compact Research-Question Status Summary.**
...
The full RQ table is provided in the appendix or supplementary material. The compact table is sufficient for the body because the main result should be read through Table 10, Table 11, and Table 12: version-level benchmark roles, paper-facing admission results, and operational recovery-routing overhead.
...
A fifth extension is a **cross-agent review signature** path for the Evidence-closure plane. This path records independent AI review as structured closure evidence rather than replacing deterministic validators, tests, or human responsibility. After an author agent produces or modifies an atom or evidence package, a reviewer agent may inspect the atom specification, diff or artifact hash, validation evidence, scope boundary, consumer contract, and rollback risk in read-only mode. The resulting review signature records reviewer identity, model/source metadata, reviewed diff hash, rubric version, verdict, reasons, and any advisory or blocking concerns. The prototype review-signature evidence path is archived under `artifacts/generated/cross-agent-review-signature/20260628/` and landed on `main` at commit `c589f125049ad81adc146f1b78e6a5ee36b8f607`. This evidence supports a narrow claim: ATM can represent cross-agent review as replayable evidence inside the Evidence-closure plane. It does not imply that an AI reviewer signature proves semantic correctness, nor that review signatures replace validators, tests, or human review. Quorum review, early warning during draft generation, and multi-reviewer disagreement resolution remain future extensions.

Sixth, **notification-guided post-admission repair** could consume broker `block`, `serial`, and fail-closed verdicts as repair triggers, drawing on CoAgent-style notification and repair while keeping ATM's core centered on pre-write admission, fail-closed unresolved cases, steward-mediated apply, and evidence closure. These providers are future work, not delivered mechanisms in the present version; each turns a strength of an adjacent system into an extension point rather than a competitive overlap.

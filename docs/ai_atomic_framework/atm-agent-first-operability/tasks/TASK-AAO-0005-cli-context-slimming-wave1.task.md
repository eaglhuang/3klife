...
title: "CLI context slimming wave 1"
status: done
owner: atm-core
priority: P1
milestone: M2
...
atomizationImpact:
  ownerAtomOrMap: "atm.agent-operability-map"
  mapUpdates:
  - "atomic_workbench/atomization-coverage/path-to-atom-map.json"
...
---
# TASK-AAO-0005 — CLI context slimming wave 1

## Goal

收斂 tasks/next 入口的上下文輸出，讓 AI 只看到當前要做的工作單。

## Why

輸出太長、太混雜時，AI 容易抓到旁支訊息開始漂移。這張卡讓 next/context 只突出目前任務與必要下一步。
...
- next output 有 playbook 摘要與完整 playbook 分層。
- planningContext/readOnlyPaths 與 targetWork/allowedFiles 清楚分離。
- 不再把 acceptance prose 混入 allowedFiles。
...
- Owner atom/map: `atm.agent-operability-map`
- Map updates:
- `atomic_workbench/atomization-coverage/path-to-atom-map.json`

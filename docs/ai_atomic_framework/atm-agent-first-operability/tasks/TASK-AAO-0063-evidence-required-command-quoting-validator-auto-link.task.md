...
(1) Normalize how `requiredCommand` strings are emitted so the same string parses cleanly under cmd.exe, PowerShell, and bash — avoiding the current escaping landmines where copy-paste of `requiredCommand` produces a different shell error than the one ATM intended. (2) When `evidence add --command "<cmd>"` is invoked and `<cmd>` matches a known validator registered in `scripts/lib/validator-envelope.ts` (e.g., `npm run validate:cli`, `npm run typecheck`), auto-link the corresponding validator name into the evidence record so `--validators` becomes optional.

## Why

Two friction loops compound when collecting close-gate evidence on Windows: PowerShell wraps/unwraps quotes inconsistently and breaks `requiredCommand` round-trips, and operators must remember to pass `--validators <name>` even when the command being recorded is unambiguous. Both are pure ergonomic wins that reduce evidence-collection failures.

## Acceptance Criteria

- `requiredCommand` strings produced by ATM round-trip across cmd.exe, PowerShell, and bash (covered by a unit test that exercises tokenization, not real shells).
...
- If auto-link risks recording a validator that did not actually run (e.g., command was `echo "npm run validate:cli"`), keep auto-link disabled until a stricter detector lands; do not ship a permissive matcher.

...
notes: "2026-06-02 | status: open | validation: pending | change: Phase 0 open card for closure packet dirty/untracked evidence hygiene guard | blocker: none | risk: evidence hygiene / dirty-tree drift"
---

# TASK-AAO-0114 closure packet dirty/untracked evidence hygiene guard

## Goal
Open the formal Phase 0 planning card for the closure packet hygiene guard and pin the close-time evidence rules before any AAF source audit.
...
This card only creates the planning anchor.
It does not clean the tree, delete noise, or touch the AAF source.
...
- Keep this as a guard / hygiene rule, not a second writer or a scratch cleanup task.

## Acceptance Case
- The close pipeline can distinguish true deliverables from unrelated working-tree noise.
...
- The follow-up AAF scope stays constrained until 005 scope audit confirms the exact source files.
...
- `C:/Users/User/AI-Atomic-Framework/packages/cli/src/commands/command-specs/tasks.spec.ts` or dedicated tests, but only after 005 scope audit confirms the close/evidence surface.
...
It does not ask us to throw the trash away right now.

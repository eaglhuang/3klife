---
doc_id: doc_other_1016
task_id: TASK-ASA-0014
...
root-drop、onefile、package dist 是 ATM 對外交付面，需納入 atom/map evidence。
...
- release-build-map
- root-drop builder atom
- onefile builder atom
- package dist builder atom

## 驗收條件

- release output 可追溯到 builder atom 與 source hash
- release parity gate 納入 dogfood score
- generated release artifacts 不被誤算為 production source debt
...
```bash
npm run build && npm run validate:root-drop-release && npm run validate:onefile-release
```

## 回滾方式

回復 release build atom/map artifacts 與 generated reports。

...
- [x] Task close fails when required evidence is missing.
- [x] Commit/PR checks expose missing evidence details.
- [x] Waiver flow is explicit and auditable.
...
2026-05-19 | status: done | validation: `npm run validate:governance-commands` + `npm run typecheck` | change: validated close, commit, and PR evidence gates including missing-review failure reporting and explicit review evidence recovery in the isolated AI-Atomic-Framework worktree | blocker: none

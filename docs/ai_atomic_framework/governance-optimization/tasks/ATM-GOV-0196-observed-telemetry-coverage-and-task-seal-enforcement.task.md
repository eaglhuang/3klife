...
deliverables:
  - atm.gateTelemetryObservedCoverageReport.v1
  - atm.gateTelemetryTaskSummary.v1 close/checkpoint enforcement
  - M3 observed cohort manifest
  - tests/cli/gate-telemetry-observed-chain.test.ts
validators:
...
- 先用本卡自己的開發流程 dogfood；若 taskflow seal/readback 尚不能被觀察，停止後卡並先修本卡。
...
- [ ] 至少一條 registered→codeWired→observed→sealed→consumed 正向鏈可重放。

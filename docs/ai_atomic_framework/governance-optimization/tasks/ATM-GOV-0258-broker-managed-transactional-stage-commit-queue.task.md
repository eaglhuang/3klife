...
This card turns the Plan 3.1 ATM-GOV-0255 and ATM-GOV-0257/0256 dogfood failure modes into first-class acceptance gates. `ATM-GOV-0259` owns editor-time write-ticket/scope guard behavior; this card must expose commit-queue surfaces that 0259 can consume without duplicating a second permission model. If one captain has release/build artifacts ready and another captain has an unrelated backlog or planning commit, ATM must serialize and isolate the stage/commit packets rather than forcing one captain to wait because the shared index could mix both actors' files. If a captain must stop while its task has valuable in-scope source WIP, ATM must fail-close release and route the lane to finish-and-close, governed non-delivery WIP commit-and-release, or auditable discard-and-release instead of leaving ownerless dirty files.
...
- [ ] Resolution-authorized protected bundles cannot deadlock release order: the current allowed task can submit its exact protected/private provenance bundle without silently unstaging or consuming another task's protected files; unauthorized use remains fail closed (`ATM-BUG-2026-07-12-134`).
- [ ] `tasks release` with dirty in-scope source files fails closed unless the actor chooses one governed recovery path: continue to finish-and-close, submit a broker-managed non-delivery WIP commit-and-release request with actor/task/lane/base SHA/allowed files/trailers, or execute discard-and-release with an auditable discard receipt. Stash and temp worktree copying are not primary-path recoveries.
- [ ] Non-delivery WIP commits are explicitly marked as WIP, cannot satisfy task validators/evidence/closeout, cannot publish release artifacts, and are rejected by final verdict as delivery evidence. They may bypass delivery-grade full validators such as repository-wide typecheck only for preservation, while still enforcing actor identity, allowed scope, protected-state rules, index isolation, touched-line split policy, and audit trailers.
...
- [ ] Runner-sync receipts, lane events, release artifacts, and post-close follow-up commits have a first-class owner/disposition and leave zero ambiguous dirty/manual-review residue (`ATM-BUG-2026-07-16-013`, `ATM-BUG-2026-07-18-005`, `ATM-BUG-2026-07-19-001`, `ATM-BUG-2026-07-19-020`, `ATM-BUG-2026-07-19-023`, `ATM-BUG-2026-07-20-211`, `ATM-BUG-2026-07-21-221`).
...
- [ ] Backlog items `ATM-BUG-2026-07-15-202`, `ATM-BUG-2026-07-15-205`, `ATM-BUG-2026-07-22-228`, `ATM-BUG-2026-07-22-229`, and `ATM-BUG-2026-07-22-235` are linked in delivery evidence and removed from Plan 3.1 open gaps only after command-backed proof shows unified broker/index admission, autonomous transactional commit/stage isolation, unrelated backlog/projection submission, and dirty-release WIP recovery.

...
- [ ] `tasks release` with dirty in-scope source files fails closed unless the actor chooses one governed recovery path: continue to finish-and-close, submit a broker-managed WIP commit-and-release request with actor/task/lane/base SHA/allowed files/trailers, or execute discard-and-release with an auditable discard receipt. Stash and temp worktree copying are not primary-path recoveries.
...
- [ ] Final metrics include dirty-release attempts, governed WIP commits, discard receipts, recovered overlapping claims, ownerless WIP blockers, manual intervention count, and false-block count.

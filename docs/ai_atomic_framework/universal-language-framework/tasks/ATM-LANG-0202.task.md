---
doc_id: doc_other_0911
task_id: ATM-LANG-0202
title: Bundled/external adapter discovery strategy
...
- [x] bundled adapter discovery policy
- [x] external adapter discovery notes
- [x] resolver fixtures for missing/available adapters
...
- 不要求 adopter 執行 install 或下載依賴才能 detection。
...
2026-05-20 | 狀態: in-progress | 驗證: pending | 變更: lock card and start bundled/external discovery strategy implementation | 阻塞: none
2026-05-20 | 狀態: done | 驗證: TS_NODE_PROJECT=tsconfig.test.json + discovery allow-list fixture（PASS）, script-facade-boundary（PASS） | 變更: 新增 discovery policy、runtime adapter readiness command、ADAPTER_GUIDE §11 | 阻塞: none

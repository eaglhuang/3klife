---
doc_id: doc_other_0912
task_id: ATM-LANG-0300
title: LegacyRoutePlan adapter delegation
...
- [x] route planning delegates to adapter when supported
- [x] adapter route report is included in guidance evidence
- [x] fallback route is explicit
...
- 核心 route plan 是 orchestrator，不是語言 parser。
...
2026-05-20 | 狀態: in-progress | 驗證: pending | 變更: lock card and start route-plan delegation implementation | 阻塞: none
2026-05-20 | 狀態: done | 驗證: node scripts/validate-guidance.ts（PASS）, TS_NODE_PROJECT=tsconfig.test.json route tests（PASS） | 變更: 新增 `planLegacyRouteWithAdapter()` 與 generic fallback route plan | 阻塞: none

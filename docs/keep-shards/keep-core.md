...
> **2026-04-29 補充**：`武將人物介面規格書 2.0` 是人物頁母本。`命 / Bloodline` 頁底部正式加入可抽拉 `StoryDock`，完整故事條落在命頁，不再預設固定掛在將頁；頁面採左側隱藏立繪 + 14 人祖先血統圖、右側命格資訊欄、底部 StoryDock 的三區配置。`命紋靈獸` 必須與 `血脈傳承奧義` 五階能力樹綁定，作為深層人格區核心。血脈成長採雙軸：`升星` 只開階段上限，`覺醒` 只點亮內容；覺醒主來源為 `名聲`，符合靈獸傾向的戰績 / 事件額外加速。史實名將升星吃同模板卡；子孫武將升星吃 `同家族卡轉化出的譜牒印記 / Lineage_Seal`。
...
1. 每次處理任何請求前，先讀 keep 入口：預設至少讀 `docs/keep.summary.md (doc_index_0012)` (doc_index_0012)；若涉及規則、架構、資料流、fallback、人物敘事、跨 repo 邊界，或本輪要修改共識，必讀 `docs/keep.md (doc_index_0011)` (doc_index_0011) 與對應分片。
...
9. 若內容同時影響系統規格與 UI 呈現，至少要同步更新主要系統規格書、`docs/UI 規格書.md (doc_ui_0027)` (doc_ui_0027) 與交叉索引。
10. 核心 `rarityTier` 與商業 `commercial band` 必須分離：前者是戰力平衡真相，後者是卡面 / 活動 / 招牌包裝的衍生結果；禁止把商業包裝回寫成核心戰力。
11. 女將商業補正只能作用於商業層，不得直接改原始五維；招牌金卡、限定卡、旗艦卡亦同。
12. **遊戲平衡不應直接截斷基本屬性**，應透過公式層（戰鬥公式 / 生產公式 / 政務公式）實現差距收斂：基本屬性保持歷史詮釋的語意正確性，公式層負責遊戲體驗的平衡性，兩者職責分離。具體做法：使用指數曲線（如 `stat^0.7`）或加權組合（如 `str*0.7 + lea*0.3`）在公式層壓縮頂端差距，而非直接調低史書名將的數值。

...
> **2026-04-29 補充**：`命 / Bloodline` 頁底部正式加入可抽拉 `StoryDock`，完整故事條落在命頁，不再預設固定掛在將頁。`命紋靈獸` 必須與 `血脈傳承奧義` 五階能力樹綁定，作為深層人格區核心。血脈成長採雙軸：`升星` 只開階段上限，`覺醒` 只點亮內容；覺醒主來源為 `名聲`，符合靈獸傾向的戰績 / 事件額外加速。史實名將升星吃同模板卡；子孫武將升星吃 `同家族卡轉化出的譜牒印記 / Lineage_Seal`。

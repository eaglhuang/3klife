...
> **2026-04-29 補充**：`武將人物介面規格書 2.0` 是人物頁母本。`命 / Bloodline` 頁底部正式加入可抽拉 `StoryDock`，完整故事條落在命頁，不再預設固定掛在將頁；頁面採左側隱藏立繪 + 14 人祖先血統圖、右側命格資訊欄、底部 StoryDock 的三區配置。`命紋靈獸` 是深層人格區核心，必須與 `血脈傳承奧義` 五階能力樹綁定。血脈成長採雙軸：`升星` 只開階段上限，`覺醒` 只點亮內容；覺醒主來源為 `名聲`，符合靈獸傾向的戰績 / 事件給額外進度。史實名將升星吃同模板卡；子孫武將升星吃 `同家族卡轉化出的譜牒印記 / Lineage_Seal`。
> **2026-05-04 補充**：**模組邊界與 Bridge 架構**正式確立。為解決 `battle/` 與 `ui/` 之間的循環引用與強耦合，強制推行「Interface-first Bridge」模式。在 `shared/interfaces/` 建立 `IBattleUIBridge` 與 `IBattleEntryParams`；`battle/` 僅可引用介面，不得直接引用 `ui/` 下的具體元件。所有跨模組通信必須透過 Bridge 注入或事件系統。此為 Harness Engineering 實踐的一部分，旨在降低 LLM 進行架構推理的複雜度。
> **2026-05-09 補充**：Claude Code 的 `SKILL.md` 規格必須使用 YAML frontmatter，且至少包含 `name` 與 `description`；`name` 需為小寫字母 / 數字 / `-`，`description` 必須明確寫出用途與觸發情境。新 skill 或同步 skill 時，三份來源（`.agents` / `.github` / `.codex`）要一起對齊。
# Keep Consensus 摘要
...
## §2. Pre-flight
- 每次執行先讀 keep 入口：預設先讀本摘要；若涉及規則、架構、資料流、fallback、人物敘事、跨 repo 邊界，或本輪要修改共識，必讀 `docs/keep.md` 與對應分片
...
- **資料與敘事不得寫死**：人物、關係、角度、證據、條件、台詞、用字不得為單一人物或 demo case 寫死；應來自上游資料、正式 pipeline 與通用規則
- **fallback 不是敘事腦**：資料不足時可回空字串 / `無資料` / unavailable，不可拿模板句、萬用句、舊 stock phrase 硬補內容；退役 fallback / 死碼要刪乾淨
- **短證據先補上下文**：原文太短時，先補上下文並抽 `人 / 事 / 時 / 地 / 物 / 情感` 結構化種子，再交給 renderer / LLM

...
## §4. 前端 / 下游顯示責任
- HTML 只負責顯示、互動、loading 狀態、timeout / abort、欄位空狀態、選項聯動。
- HTML 不負責人物性格判斷、關係修正、角度修正、證據補腦、旁人感想生成、劇情補完。
- 若 scene 資料錯、關係錯、角度錯，前端只顯示可用狀態與診斷結果，不在本 repo 寫人物特判遮錯。
- 下游若某欄無資料，可依畫面契約顯示空字串或明確空狀態，但不能自行拼故事。

...
export interface CSharpModuleAnalysis {
  ...
  warnings: string[];
  usingAliases: Record<string, string>;
  staticUsings: string[];
  ...
  const parameterKinds = normalized
    ...
    .map((segment) => {
      ...
      const tokens = withoutAttributes
        ...
        .filter(Boolean)
        .filter((token) => !['ref', 'out', 'in', 'params', 'scoped'].includes(token));
      const isExtensionParameter = tokens[0] === 'this';
      const typeTokens = isExtensionParameter ? tokens.slice(1) : tokens;
      const typeValue = typeTokens.length <= 1
        ? canonicalizeSymbolToken(typeTokens[0] ?? withoutAttributes)
        : canonicalizeSymbolToken(typeTokens.slice(0, -1).join(' '));
...
function parseModule(filePath: string, content: string): CSharpModuleAnalysis {
  ...
  const usingAliases: Record<string, string> = {};
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const usingAliasMatch = trimmed.match(
      /^(global\s+)?using\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*([A-Za-z0-9_\.]+)\s*;/
    );
    if (usingAliasMatch) {
      const alias = cleanIdentifier(usingAliasMatch[2]);
      const target = cleanIdentifier(usingAliasMatch[3]);
      usingAliases[alias] = target;
      dependencyEdges.push({
        from: filePath,
        to: `module:${target}`,
        relation: usingAliasMatch[1] ? 'imports-alias-global' : 'imports-alias',
        evidence: `${filePath}:${lineNo}`,
      });
    }

    const usingStaticMatch = trimmed.match(/^(global\s+)?using\s+static\s+([A-Za-z0-9_\.]+)\s*;/);
    if (usingStaticMatch) {
      const target = cleanIdentifier(usingStaticMatch[2]);
      staticUsings.add(target);
      dependencyEdges.push({
        from: filePath,
        to: `module:${target}`,
        relation: usingStaticMatch[1] ? 'imports-static-global' : 'imports-static',
        evidence: `${filePath}:${lineNo}`,
      });
    }
    ...
    const methodMatch = trimmed.match(
      /^(?:\[[^\]]+\]\s*)*(?:public|private|protected|internal|static|virtual|override|sealed|new|async|extern|partial|\s)+\s+[A-Za-z_][A-Za-z0-9_<>,\[\]\?\.]*\s+([A-Za-z_][A-Za-z0-9_]*)(\s*<[^>]+>)?\s*\(([^\)]*)\)\s*(?:\{|=>|where|$|;)/
    );
    let declaredMethodName: string | undefined;
    if (methodMatch) {
      const methodName = cleanIdentifier(methodMatch[1]);
      const methodGenericSuffix = methodMatch[2]?.replace(/\s+/g, '') ?? '';
      ...
      if (!CALL_KEYWORDS.has(methodName)) {
        ...
        const displayName = buildDisplayName(
          namespaceName,
          typePath,
          `${methodName}${methodGenericSuffix}`
        );
        const signature = buildMethodSignature(methodMatch[3] ?? '');
        const symbolId = allocateSymbolId({
          kind: 'method',
          displayName,
          signature,
  ...
  return {
    ...
    warnings,
    usingAliases,
    staticUsings: Array.from(staticUsings).sort((left, right) => left.localeCompare(right)),
    typeEvidence,
    generatedEvidence,
  };
}

...
const TARGET_CODE = 'ATM_TASK_AUDIT_LEGACY_BASELINE_DONE';
...
try {
  ...
  console.error('LEGACY_BASELINE_PARSE_ERROR');
  console.error(error.message);
  process.exit(1);
}

const findings = audit.evidence?.report?.findings || [];
const legacy = findings.filter((finding) => finding.code === TARGET_CODE);
const active = legacy.filter((finding) => finding.acknowledged !== true);
...
console.log(`LEGACY_BASELINE_TOTAL=${legacy.length}`);
console.log(`LEGACY_BASELINE_ACTIVE=${active.length}`);
...
if (active.length > 0) {
  console.error('Unacknowledged legacy-baseline findings remain:');

...
const manualDone = findings.filter(
  (finding) => finding.code === "ATM_TASK_AUDIT_MANUAL_DONE",
);

console.log(`MANUAL_DONE=${manualDone.length}`);
console.log(`TOTAL_FINDINGS=${findings.length}`);

if (manualDone.length > 0) {
  for (const finding of manualDone) {

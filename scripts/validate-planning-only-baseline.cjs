...
const TARGET_CODE = 'ATM_TASK_AUDIT_PLANNING_ONLY_DONE';
...
try {
  ...
  console.error('PLANNING_ONLY_BASELINE_PARSE_ERROR');
  console.error(error.message);
  process.exit(1);
}

const findings = audit.evidence?.report?.findings || [];
const planningOnly = findings.filter((finding) => finding.code === TARGET_CODE);
const active = planningOnly.filter((finding) => finding.acknowledged !== true);
...
console.log(`PLANNING_ONLY_TOTAL=${planningOnly.length}`);
console.log(`PLANNING_ONLY_ACTIVE=${active.length}`);
...
if (active.length > 0) {
  console.error('Unacknowledged planning-only findings remain:');

...
function run(command, args) {
  step += 1;
  const outPath = path.join(workdir, `compile-en-${String(step).padStart(2, '0')}-${command}.out`);
...
run('xelatex', ['-interaction=nonstopmode', 'paper-en.tex']);
run('xelatex', ['-interaction=nonstopmode', 'paper-en.tex']);

const pdfPath = path.join(workdir, 'paper-en.pdf');
const logPath = path.join(workdir, 'paper-en.log');
...
if (errors > 0) {
  for (const line of log.split(/\r?\n/).filter((x) => x.startsWith('!')).slice(0, 8)) {
    console.log(line);
  }
  process.exitCode = 1;
}

...
const crestRingV7 = findImage('crest_ring_v7_muted_');
const storyV7 = findImage('story_strip_v7_rich_');
// We use V6 face but apply desaturation filters
const crestFaceV6 = findImage('crest_face_v6_'); 
const badgeV6 = findImage('rarity_badge_v6_');

console.log("V7 Found:", {crestRingV7, storyV7, crestFaceV6, badgeV6});

const JOBS = [
    { target: 'crest_ring_v5.png', src: crestRingV7, w: 220, h: 220, type: 'fit_circle' },
    { target: 'crest_face_v5.png', src: crestFaceV6, w: 180, h: 180, type: 'filtered_watermark' },
    ...
    { target: 'story_strip_base_v5.png', src: storyV7, w: 600, h: 100, type: 'stretch' }
];
...
async function main() {
    ...
    try {
        for (const job of JOBS) {
            ...
            if (job.type === 'filtered_watermark') {
                // Apply a heavy filter to turn the face into a charcoal/grey muted watermark to remove all yellow
                filter = "filter: grayscale(100%) opacity(70%) brightness(85%) contrast(120%) sepia(20%) hue-rotate(150deg);";
                ...
                filter = "filter: grayscale(40%) opacity(85%);"; // mute badges more
    ...
    console.log('V7 Ultimate Polish 完成');
}
